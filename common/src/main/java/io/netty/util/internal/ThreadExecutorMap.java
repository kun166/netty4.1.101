...
public final class ThreadExecutorMap {
     ...
     * <p>
     * 看不懂，为啥这么写……
     * 在{@link SingleThreadEventExecutor#SingleThreadEventExecutor(io.netty.util.concurrent.EventExecutorGroup, java.util.concurrent.Executor, boolean, java.util.Queue, io.netty.util.concurrent.RejectedExecutionHandler)}
     * 构造器里面，被调用
     * executor是在{@link }
     *
     * @param executor      实际执行任务的Executor
     * @param eventExecutor 调用{@link #currentExecutor()}需要返回的EventExecutor
     * @return
     */
     ...
     * when called from within the {@link Runnable} during execution.
     *
     * @param command       被执行的{@link Runnable}
     * @param eventExecutor 执行任务过程中,调用{@link #currentExecutor()} 需要返回的EventExecutor
     * @return
     */
