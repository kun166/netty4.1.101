...
abstract class AbstractChannelHandlerContext implements ChannelHandlerContext, ResourceLeakHint {
    ...
    // 该ChannelHandlerContext包装的ChannelHandler的所有方法中非标注@Skip的方法的mask值的并集
    ...
    private void invokeChannelRead(Object msg) {
        if (invokeHandler()) {
            try {
                ...
                // Duplex handlers implements both out/in interfaces causing a scalability issue
                // see https://bugs.openjdk.org/browse/JDK-8180450
                // 获取持有的ChannelHandler 对象
                final ChannelHandler handler = handler();
                // DefaultChannelPipeline 的头节点
                final DefaultChannelPipeline.HeadContext headContext = pipeline.head;
                if (handler == headContext) {
                    // 如果是头节点，则依此向下找
    ...
    /**
     * {@link io.netty.channel.DefaultChannelPipeline#bind(java.net.SocketAddress, io.netty.channel.ChannelPromise)}调用
     *
     * @param localAddress
     * @param promise
     * @return
    ...
    /**
     * 从当前的ChannelHandlerContext往前找(不包括当前),因为这个是outbound,所以是从tail往前找，直到head,
     * 找实现了传入了mask值的那个方法，且该方法上未标注@Skip注解
     *
     * @param mask
     * @return
     */
    ...
    /**
     * 是否跳过ctx持有的ChannelHandle的mask值对应的方法
     *
     * @param ctx             当前AbstractChannelHandlerContext
     * @param currentExecutor EventExecutor
     * @param mask            需要执行的方法对应的mask值
     * @param onlyMask
     * @return
     */
    private static boolean skipContext(
        ...
        // ctx.executionMask 表示的是ctx持有的ChannelHandle的方法未标记@Skip的值
        // (ctx.executionMask & mask) == 0,说明该ctx包装的ChannelHandler的mask对应的方法上标注了@Skip注解
    ...
    /**
     * 在{@link AbstractChannelHandlerContext#callHandlerAdded()}中被调用
     *
     * @return
     */
    ...
    /**
     * 在{@link DefaultChannelPipeline#addFirst(io.netty.util.concurrent.EventExecutorGroup, java.lang.String, io.netty.channel.ChannelHandler)}
     * 中被调用。
     * 这个方法在DefaultChannelPipeline的add**方法里均有调用
     */
    ...
    /**
     * 在{@link DefaultChannelPipeline#callHandlerAdded0(io.netty.channel.AbstractChannelHandlerContext)}中被调用
     *
     * @throws Exception
     */
    final void callHandlerAdded() throws Exception {
        ...
        if (setAddComplete()) {
            /**
             * 在{@link ChannelInitializer#handlerAdded(io.netty.channel.ChannelHandlerContext)}中有实现
             */
